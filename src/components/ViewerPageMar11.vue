<template>
    ...
    <q-card class="col-auto" style="width: 700px;height: 780px;">
      <q-card-section style="height: 780px;">
        <DataTable v-if="uploadedFileContent" :uploadedFileContent="uploadedFileContent" :fileName="uploadedFile.name" @set-data="onSubmitDataTable" @save-request="handleSubmitData"  @dataToSubmit="handleEnteredValues"/>
      </q-card-section>
    </q-card>
  ...
  console.log('Entered values from DataTable:', dataToSubmit); // Ensure this line is correctly placed
    ...
    async uploadAndParseFile() {
      ...
      try {
        ...
        const response = await axios.post('http://172.18.100.240:6767/parse_dxf/', formData, {
          headers: {
